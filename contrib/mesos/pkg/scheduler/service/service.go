...
func (s *SchedulerServer) prepareExecutorInfo(hks hyperkube.Interface) (*mesos.ExecutorInfo, error) {
	...
	if s.staticPodsConfigPath != "" {
		...
		for _, podPath := range paths {
			...
			if err != nil {
				return nil, fmt.Errorf("error reading static pod spec: %v", err)
			}

			pod := api.Pod{}
			err = json.Unmarshal(podJson, &pod)
			if err != nil {
				return nil, fmt.Errorf("error parsing static pod spec at %v: %v", podPath, err)
			}

			_, cpu, _, err := mresource.LimitPodCPU(&pod, s.defaultContainerCPULimit)
			if err != nil {
				return nil, fmt.Errorf("cannot derive cpu limit for static pod: %v", podPath)
			}
			_, mem, _, err := mresource.LimitPodMem(&pod, s.defaultContainerMemLimit)
			if err != nil {
				return nil, fmt.Errorf("cannot derive memory limit for static pod: %v", podPath)
			}
