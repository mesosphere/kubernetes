...
func TestEmptyOffer(t *testing.T) {
	...
	if err := defaultProc.Procure(task, &mesos.Offer{}, &api.Node{}, &Spec{}); err == nil {
...
func TestNoPortsInPodOrOffer(t *testing.T) {
	...
	if err := defaultProc.Procure(task, offer, nil, &Spec{}); err == nil {
...
func TestNodeSelector(t *testing.T) {
	...
	for _, ts := range tests {
		...
		err := defaultProc.Procure(task, offer, ts.node, &Spec{})
		ok := err == nil
		if ts.ok != ok {
