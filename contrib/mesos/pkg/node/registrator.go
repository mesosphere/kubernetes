	...
	RegistrationLoop:
		for {
			...
			if n == nil {
				...
				_, err := CreateOrUpdate(r.client, rg.hostName, rg.labels, map[string]string{})
				if err != nil {
				...
				_, err := Update(r.client, n, rg.labels, map[string]string{})
				if err != nil && errors.IsNotFound(err) {
					// last chance when our store was out of date
					_, err = Create(r.client, rg.hostName, rg.labels, map[string]string{})
				}
				if err != nil {
